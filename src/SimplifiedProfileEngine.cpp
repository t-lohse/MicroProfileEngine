...
bool heating_finished() { return true; }

bool has_reached_final_weight() { return false; }

void setTargetWeight(double setPoint) { printf("Setting target weight to %f\n", setPoint); }

void setTargetTemperature(double setPoint) { printf("Setting target temperature to %f\n", setPoint); }

void setTargetPressure(double setPoint) { printf("Setting target pressure to %f\n", setPoint); }

void setLimitedPressure(double setPoint) { printf("Setting target pressure limit to %f\n", setPoint); }

void setTargetFlow(double setPoint) { printf("Setting target flow to %f\n", setPoint); }

void setLimitedFlow(double setPoint) { printf("Setting flow limit to %f\n", setPoint); }

void setTargetPower(double setPoint) { printf("Setting target power to %f\n", setPoint); }

void setTargetPistonPosition(double setPoint) { printf("Setting target piston position to %f\n", setPoint); }
...
{
    printf("Saving %s log for stage %ld. Timestamp = %ld\n", is_stage_exit == STAGE_ENTRY ? "START" : "EXIT",
           this->currentStageId, timestamp);
...
void SimplifiedProfileEngine::step()
{
    ...
    switch (this->state) {
    case ProfileState::IDLE: break;
    case ProfileState::START: this->state = ProfileState::HEATING; break;

    case ProfileState::HEATING:
        setTargetTemperature(parseProfileTemperature(this->profile->temperature));
    ...
    case ProfileState::RETRACTING:
        ...
        break;
    case ProfileState::BREWING: this->state = this->processStageStep(); break;
...
ProfileState SimplifiedProfileEngine::processStageStep()
{
    ...
    auto stage_timestamp =
        (now - (this->profileStartTimestamp + (log->start.timestamp * std::chrono::milliseconds(1)))) /
    ...
    case InputType::INPUT_TIME: input_reference_value = stage_timestamp; break;
    case InputType::INPUT_PISTON_POSITION:
        input_reference_value = this->driver->get_sensor_data().piston_position;
        break;
    case InputType::INPUT_WEIGHT: input_reference_value = this->driver->get_sensor_data().weight; break;
    default: printf("Invalid input reference type! Aborting"); return ProfileState::ERROR;
    }
    ...
    case ControlType::CONTROL_PRESSURE: setTargetPressure(sampled_output); break;
    case ControlType::CONTROL_FLOW: setTargetFlow(sampled_output); break;
    case ControlType::CONTROL_POWER: setTargetPower(sampled_output); break;
    case ControlType::CONTROL_PISTON_POSITION: setTargetPistonPosition(sampled_output); break;
    default: printf("Invalid control method! Aborting"); return ProfileState::ERROR;
    }
