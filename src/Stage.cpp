...
const Dynamics& Stage::getDynamics() const {
    return dynamics;
}
// Const vals
const std::vector<ExitTrigger>& Stage::getExitTriggers() const {
    return exitTriggers;
}
const std::vector<Limit>& Stage::getLimits() const {
    return limits;
}
const ControlType Stage::getControlType() const {
    return controlType;
}
...
const std::chrono::system_clock& StageVariables::getTimestamp() const {
        return timestamp;
    }

    // StageLog
    bool StageLog::isValid() const {
